...
.\bootstrap-vcpkg.bat
```
### 🛠️ How to Build the Project (Local Development)

This project uses `vcpkg` for dependency management. To compile and run the application on your machine, please follow these steps:

**Step 1: Update the Toolchain Path**
1. Open the `CMakePresets.json` file located in the root of the project.
2. Locate the `CMAKE_TOOLCHAIN_FILE` variable.
3. Change its value to the absolute path of your local `vcpkg` installation. 
   *(Example: `"C:/vcpkg/scripts/buildsystems/vcpkg.cmake"`)*

**Step 2: Configure and Build**
Open your terminal (PowerShell or Command Prompt), navigate to the project directory, and run the following standard commands:
...
# 3. Configure the project (This will automatically read your CMakePresets.json)
cmake ..

# 4. Compile the application (Defaults to Debug mode)
cmake --build .
...
```powershell
cd Code/Chatties
cmake -B build -S . -DCMAKE_PREFIX_PATH="...\Qt\6.11.1\msvc2022_64"
cmake --build build
```
